...
# --- 4. 注入自定义图标与标题（当前文档 + 父文档，并延迟再次注入以覆盖 Streamlit 默认）---
if _final_icon_b64:
    _data_url = f"data:image/png;base64,{_final_icon_b64}"
    _app_title_js = json.dumps(APP_TITLE)  # 避免中文在 JS 中出问题
    st.markdown(
        f"""
<script>
(function(){{
  var dataUrl = {json.dumps(_data_url)};
  var appTitle = {_app_title_js};
  function applyIconAndTitle(doc) {{
    if (!doc || !doc.head) return;
    function forceUpdateTag(rel, href) {{
      var existingLinks = doc.querySelectorAll("link[rel*='" + rel + "']");
      existingLinks.forEach(function(el) {{ if (el.parentNode) el.parentNode.removeChild(el); }});
      var newLink = doc.createElement("link");
      newLink.rel = rel;
      newLink.href = href;
      doc.head.appendChild(newLink);
    }}
    forceUpdateTag("apple-touch-icon", dataUrl);
    forceUpdateTag("apple-touch-icon-precomposed", dataUrl);
    forceUpdateTag("shortcut icon", dataUrl);
    forceUpdateTag("icon", dataUrl);
    ...
    if (!metaTitle) {{ metaTitle = doc.createElement("meta"); metaTitle.name = "apple-mobile-web-app-title"; doc.head.appendChild(metaTitle); }}
    metaTitle.content = appTitle;
  }}
  applyIconAndTitle(document);
  applyIconAndTitle(window.parent && window.parent.document ? window.parent.document : null);
  setTimeout(function() {{ applyIconAndTitle(document); applyIconAndTitle(window.parent && window.parent.document ? window.parent.document : null); }}, 800);
}})();
</script>
""",
...
/* 底部导航为 Streamlit 按钮行时：紧跟 #icon-navbar 的横向块固定为绿色胶囊样式 */
#icon-navbar + [data-testid="stHorizontalBlock"] {
...
#icon-navbar + [data-testid="stHorizontalBlock"] .stButton > button {
    border-radius: 14px !important;
...
def render_icon_navbar(active_page: str) -> None:
    """底部导航：用按钮切换 session_state['page'] 并 rerun，不改 URL，避免整页重载。"""
    st.markdown('<div class="fixed-nav-spacer"></div>', unsafe_allow_html=True)
    st.markdown(
        '<div id="icon-navbar" style="display:flex; flex-wrap:nowrap; justify-content:space-between; align-items:center; gap:0;">',
        unsafe_allow_html=True,
    )
    c1, c2, c3, c4 = st.columns(4)
    nav_items = [
        ("home", "首页", "nav_home"),
        ("fridge", "冰箱", "nav_fridge"),
        ("discover", "发现", "nav_discover"),
        ("mine", "我的", "nav_mine"),
    ]
    for col, (page, label, key) in zip([c1, c2, c3, c4], nav_items):
        with col:
...
# 页面切换仅用 session_state，不再用 URL 参数，避免点击导航时浏览器整页重载
try:
