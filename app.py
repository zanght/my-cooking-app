...
def _app_icon_base64() -> str:
    """
    优先读取 assets/app_icons/app_icon_180.png，不存在则用 assets/photo.png。
    若非正方形则居中裁切为正方形，再统一缩放到 180x180，转 PNG base64 供 Apple 主屏幕图标用。
    """
    path = None
    for p in (
        ASSETS_DIR / "app_icons" / "app_icon_180.png",
    ...
    if img.size[0] != 180 or img.size[1] != 180:
...
# 注入到 document.head，使 iOS「添加到主屏幕」能显示自定义图标（Streamlit 无法直接改 head，用脚本追加）
_icon_b64 = _app_icon_base64()
if _icon_b64:
