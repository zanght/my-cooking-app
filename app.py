...
<script>
(function(){{
  // 关键：修改 window.parent.document (即浏览器最外层，避开 Streamlit 的 Iframe 限制)
  var targetDoc = window.parent.document;
  var dataUrl = "{_data_url}";

  // 定义一个函数来强制更新或创建标签，并移除旧的冲突标签
  function forceUpdateTag(rel, href) {{
    var existingLinks = targetDoc.querySelectorAll("link[rel*='" + rel + "']");
    existingLinks.forEach(function(el) {{ el.parentNode.removeChild(el); }});
    
    var newLink = targetDoc.createElement("link");
    newLink.rel = rel;
    newLink.href = href;
  ...
  // 2. 强制覆盖网页标签页图标 (Favicon)，打败红色 Logo
  forceUpdateTag("shortcut icon", dataUrl);
  forceUpdateTag("icon", dataUrl);

  // 3. 开启全屏 Web App 沉浸模式
  var meta = targetDoc.querySelector("meta[name='apple-mobile-web-app-capable']");
  if (!meta) {{
  ...
  // 4. 设置 App 添加到主屏幕时的默认显示名称
  var metaTitle = targetDoc.querySelector("meta[name='apple-mobile-web-app-title']");
  if (!metaTitle) {{
    metaTitle = targetDoc.createElement("meta");
    metaTitle.name = "apple-mobile-web-app-title";
    targetDoc.head.appendChild(metaTitle);
  }}
  metaTitle.content = "{APP_TITLE}";
}})();
</script>
""",
