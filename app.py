...
# --- 1. 预先处理图标对象 (用于网页标签页) ---
def _get_app_icon_obj():
    """获取图标对象，优先使用 180 尺寸的图"""
    for p in (ASSETS_DIR / "app_icons" / "app_icon_180.png", ASSETS_DIR / "photo.png"):
        if p.exists():
            try:
                return Image.open(p)
            except:
...
app_icon_obj = _get_app_icon_obj()

# --- 2. 页面配置 (必须是第一个 Streamlit 命令)；page_icon 需为字符串路径或 emoji ---
_page_icon = (
    str(ASSETS_DIR / "app_icons" / "app_icon_180.png")
    if (ASSETS_DIR / "app_icons" / "app_icon_180.png").exists()
    else (str(ASSETS_DIR / "photo.png") if (ASSETS_DIR / "photo.png").exists() else "🍳")
)
st.set_page_config(
...
# --- 3. 生成用于强力覆盖的高质量 Base64 数据 ---
def _generate_final_icon_b64() -> str:
    if isinstance(app_icon_obj, Image.Image):
        try:
            img = app_icon_obj.convert("RGBA")
            # 居中裁剪为正方形
            w, h = img.size
            if w != h:
                size = min(w, h)
            ...
            # 缩放到标准 180px
            img = img.resize((180, 180), getattr(Image, "Resampling", Image).LANCZOS)
            buf = io.BytesIO()
...
_final_icon_b64 = _generate_final_icon_b64()

# --- 4. 主屏幕图标：用 window.parent.document 删除 Streamlit 红色图标，插入 Base64 恐龙图标；1 秒延迟执行 ---
if _final_icon_b64:
    _data_url = f"data:image/png;base64,{_final_icon_b64}"
    _title_esc = APP_TITLE.replace("&", "&amp;").replace('"', "&quot;").replace("<", "&lt;")
    st.markdown(
        f'<div id="app-icon-payload" data-url="{_data_url}" data-title="{_title_esc}" style="display:none;"></div>',
...
<script>
(function(){
  var el = document.getElementById("app-icon-payload");
  if (!el) return;
  var dataUrl = el.getAttribute("data-url");
  var appTitle = el.getAttribute("data-title") || "很好吃";
  if (!dataUrl) return;
  ...
  function applyToParent() {
    ...
    function removeRel(rel) {
      var list = doc.querySelectorAll("link[rel*='" + rel + "']");
      for (var i = 0; i < list.length; i++) if (list[i].parentNode) list[i].parentNode.removeChild(list[i]);
    }
    removeRel("apple-touch-icon");
    removeRel("apple-touch-icon-precomposed");
    removeRel("shortcut icon");
    removeRel("icon");
    ...
    addLink("apple-touch-icon", dataUrl, "180x180");
    addLink("apple-touch-icon-precomposed", dataUrl, "180x180");
    addLink("shortcut icon", dataUrl);
    addLink("icon", dataUrl);
    try { doc.title = appTitle; } catch(e) {}
    var meta = doc.querySelector('meta[name="apple-mobile-web-app-capable"]');
    if (!meta) { meta = doc.createElement("meta"); meta.name = "apple-mobile-web-app-capable"; doc.head.appendChild(meta); }
    meta.content = "yes";
    var t = doc.querySelector('meta[name="apple-mobile-web-app-title"]');
    if (!t) { t = doc.createElement("meta"); t.name = "apple-mobile-web-app-title"; doc.head.appendChild(t); }
    t.content = appTitle;
  }
  setTimeout(applyToParent, 1000);
})();
</script>
""",
        unsafe_allow_html=True,
...
#icon-navbar a:hover,
#icon-navbar .nav-item:hover {
    transform: scale(1.03);
    background: transparent;
}
#icon-navbar a:active,
#icon-navbar .nav-item:active {
    transform: scale(0.9);
}
#icon-navbar img {
    height: 24px;
...
#icon-navbar .nav-item.active img {
    filter: drop-shadow(0 0 4px rgba(255, 255, 255, 0.8))
            drop-shadow(0 2px 5px rgba(0, 0, 0, 0.18));
}
/* 透明按钮层：紧跟 nav-bar-wrap 的 Streamlit 容器，覆盖在导航图标上，仅响应点击 */
div:has(#nav-bar-wrap) + div,
#nav-bar-wrap + div {
...
div:has(#nav-bar-wrap) + div > div,
#nav-bar-wrap + div > div {
    flex: 1 !important;
    min-width: 0 !important;
    pointer-events: auto !important;
}
div:has(#nav-bar-wrap) + div .stButton > button,
#nav-bar-wrap + div .stButton > button {
    background: transparent !important;
    border: none !important;
    opacity: 0 !important;
    width: 100% !important;
...
def render_icon_navbar(active_page: str) -> None:
    ...
    # 视觉层：保留原有浅绿胶囊、图标与文字，用 div.nav-item 替代 a；nav-bar-wrap 用于 CSS 定位紧跟的按钮层
    st.markdown(
        f"""
<div id="nav-bar-wrap">
...
""",
    ...
    with c1:
        if st.button("首页", key="nav_home", use_container_width=True):
            ...
            st.rerun()
    with c2:
        if st.button("冰箱", key="nav_fridge", use_container_width=True):
            st.session_state["page"] = "fridge"
            try:
                st.query_params.update(page="fridge")
            except Exception:
                pass
            st.rerun()
    with c3:
        if st.button("发现", key="nav_discover", use_container_width=True):
            st.session_state["page"] = "discover"
            try:
                st.query_params.update(page="discover")
            except Exception:
                pass
            st.rerun()
    with c4:
        if st.button("我的", key="nav_mine", use_container_width=True):
            st.session_state["page"] = "mine"
            try:
                st.query_params.update(page="mine")
            except Exception:
                pass
            st.rerun()
...
# 页面切换：优先从 URL 参数 page 恢复当前页，然后由底部导航按钮更新 session_state + st.query_params
try:
    _q_page = st.query_params.get("page")
    if _q_page in ("home", "fridge", "discover", "mine"):
        st.session_state["page"] = _q_page
except Exception:
    pass
